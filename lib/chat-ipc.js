...
function normalizeChatPayload(payload) {
  ...
  const maxTokens = Number.isFinite(requestedMaxTokens)
    ? Math.min(Math.max(Math.round(requestedMaxTokens), 1), 4096)
    : 150;
  return {
    baseUrl,
...
function describeEmptyOpenAIResponse(json) {
  const message = json?.choices?.[0]?.message;
  if (typeof message?.reasoning_content === 'string' && message.reasoning_content.trim()) {
    return 'Model returned reasoning only without final content';
  }
  ...
  function registerIpc() {
    ipcMain.handle('chat', async (event, payload) => {
      try {
        ...
        const { baseUrl, model, apiKey, messages, provider, maxTokens, requestId } = normalizedPayload;
            ...
            res.on('end', () => {
              if (res.statusCode < 200 || res.statusCode >= 300) {
                ...
                try {
                  ...
                  errMsg =
                    json.error?.message || json.error?.msg || json.error?.type || JSON.stringify(json).slice(0, 300);
                } catch {
              ...
              try {
                ...
                if (isAnthropic) {
                  ...
                  const text = extractOpenAIMessageContent(json);
                  if (text) {
                    resolve(text);
                    return;
                  }
                  reject(new Error(describeEmptyOpenAIResponse(json)));
                }
              } catch (e) {
