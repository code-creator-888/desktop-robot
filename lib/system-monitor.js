...
function createSystemMonitor({ ipcMain, execFileAsync, getCachedValue, cacheTtlMs = 1500 }) {
  ...
  async function getNetStats() {
    try {
      ...
      for (const line of stdout.split('\n')) {
        ...
        if (columns.length < 10 || columns[0] === 'Name' || columns[0].startsWith('lo') || columns[3] === 'Address')
          continue;
  ...
  async function getDiskStats() {
    try {
      const { stdout } = await execFileAsync(
        '/usr/sbin/ioreg',
        ['-c', 'IOBlockStorageDriver', '-r', '-k', 'Statistics'],
        { timeout: 10000 }
      );
      let totalRead = 0,
        totalWrite = 0;
  ...
  async function buildSystemStats() {
    const [processes, netStats, diskStats] = await Promise.all([getTopProcesses(), getNetStats(), getDiskStats()]);
