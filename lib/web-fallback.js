...
function parseBingResults(html) {
  const blocks = [
    ...String(html || '').matchAll(
      /<li class="b_algo"[\s\S]*?<h2[^>]*>\s*<a[^>]*href="([^"]+)"[^>]*>([\s\S]*?)<\/a>\s*<\/h2>[\s\S]*?(?:<div class="b_caption"[^>]*>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>[\s\S]*?<\/div>)?/g
    )
  ];
  return blocks.map((m) => ({
    url: decodeHtmlEntities(m[1]),
...
function parseSoResults(html) {
  const blocks = [
    ...String(html || '').matchAll(
      /<li class="res-list"[\s\S]*?<h3 class="res-title[^"]*"[^>]*>\s*<a[^>]*href="([^"]+)"(?:[^>]*data-mdurl="([^"]+)")?[^>]*>([\s\S]*?)<\/a>\s*<\/h3>[\s\S]*?(?:<p class="res-desc"[^>]*>([\s\S]*?)<\/p>)?/g
    )
  ];
  return blocks.map((m) => ({
